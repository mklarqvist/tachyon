...
VariantBlockFooter::VariantBlockFooter(const self_type& other) :
	...
	filter_pattern_map(nullptr)
{
	for(U32 i = 0; i < YON_BLK_N_STATIC; ++i) this->offsets[i] = other.offsets[i];

	for(U32 i = 0; i < this->n_info_streams; ++i)   this->info_offsets[i]   = other.info_offsets[i];
	for(U32 i = 0; i < this->n_format_streams; ++i) this->format_offsets[i] = other.format_offsets[i];
	for(U32 i = 0; i < this->n_filter_streams; ++i) this->filter_offsets[i] = other.filter_offsets[i];

	for(U32 i = 0; i < this->n_info_patterns; ++i)   this->info_patterns[i]   = other.info_patterns[i];
	for(U32 i = 0; i < this->n_format_patterns; ++i) this->format_patterns[i] = other.format_patterns[i];
	for(U32 i = 0; i < this->n_filter_patterns; ++i) this->filter_patterns[i] = other.filter_patterns[i];
...
void VariantBlockFooter::AllocateInfoHeaders(const U32 n_info_streams){
	delete [] this->info_offsets;
	if(n_info_streams == 0){
...
void VariantBlockFooter::AllocateFormatHeaders(const U32 n_format_streams){
	delete [] this->format_offsets;
...
void VariantBlockFooter::AllocateFilterHeaders(const U32 n_filter_streams){
	delete [] this->filter_offsets;
...
void VariantBlockFooter::AllocateHeaders(const U32 n_info_streams,
...
bool VariantBlockFooter::ConstructInfoBitVector(std::unordered_map<U32,U32>* pattern_map){
...
bool VariantBlockFooter::ConstructFormatBitVector(std::unordered_map<U32,U32>* pattern_map){
...
bool VariantBlockFooter::ConstructFilterBitVector(std::unordered_map<U32,U32>* pattern_map){
...
U32 VariantBlockFooter::AddPatternWrapper(const std::vector<int>& pattern,
...
U32 VariantBlockFooter::AddInfoPattern(const std::vector<int>& pattern){
...
U32 VariantBlockFooter::AddFormatPattern(const std::vector<int>& pattern){
...
U32 VariantBlockFooter::AddFilterPattern(const std::vector<int>& pattern){
...
// disk/buffer.
U32 VariantBlockFooter::UpdatePatternWrapper(const std::vector<int>& pattern,
					  map_pattern_type* pattern_map,
...
U32 VariantBlockFooter::UpdateInfoPattern(const std::vector<int>& pattern, const U16 pattern_id){
	if(this->info_pattern_map == nullptr) this->BuildPatternMaps();
	if(this->n_info_patterns_allocated == 0){
		delete [] this->info_patterns;
		this->info_patterns = new yon_blk_bv_pair[100];
...
U32 VariantBlockFooter::UpdateFormatPattern(const std::vector<int>& pattern, const U16 pattern_id){
	if(this->format_pattern_map == nullptr) this->BuildPatternMaps();
	if(this->n_format_patterns_allocated == 0){
		delete [] this->format_patterns;
		this->format_patterns = new yon_blk_bv_pair[100];
...
U32 VariantBlockFooter::UpdateFilterPattern(const std::vector<int>& pattern, const U16 pattern_id){
	if(this->filter_pattern_map == nullptr) this->BuildPatternMaps();
	if(this->n_filter_patterns_allocated == 0){
		delete [] this->filter_patterns;
		this->filter_patterns = new yon_blk_bv_pair[100];
...
U32 VariantBlockFooter::UpdateOffsetMapWrapper(const header_type& offset, map_type* map, const U16& stream_counter){
...
U32 VariantBlockFooter::UpdateInfo(const header_type& offset, const U16 position){
	if(this->info_map == nullptr) this->BuildMaps();
	return(this->UpdateOffsetMapWrapper(offset, this->info_map, position));
}

U32 VariantBlockFooter::UpdateFormat(const header_type& offset, const U16 position){
	if(this->format_map == nullptr) this->BuildMaps();
	return(this->UpdateOffsetMapWrapper(offset, this->format_map, position));
}

U32 VariantBlockFooter::UpdateFilter(const header_type& offset, const U16 position){
	if(this->filter_map == nullptr) this->BuildMaps();
	return(this->UpdateOffsetMapWrapper(offset, this->filter_map, position));
}

U32 VariantBlockFooter::AddStreamWrapper(const U32 id, map_type* map, header_type*& offsets, U16& stream_counter){
