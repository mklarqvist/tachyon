...
public:
    AbstractIntegerContainer(const data_container_type& container);
	...
	inline iterator end(){ return iterator(&this->__iterators[this->n_entries]); }
	inline const_iterator begin() const{ return const_iterator(&this->__iterators[0]); }
	inline const_iterator end() const{ return const_iterator(&this->__iterators[this->n_entries]); }
	inline const_iterator cbegin() const{ return const_iterator(&this->__iterators[0]); }
	inline const_iterator cend() const{ return const_iterator(&this->__iterators[this->n_entries]); }
...
private:
	template <class actual_primitive> void __setup(const data_container_type& container, nextStrideFunction func){
	...
	template <class actual_primitive> void __setup(const data_container_type& container, const U32 stride_size){
...
template <class return_primitive>
AbstractIntegerContainer<return_primitive>::AbstractIntegerContainer(const data_container_type& container) :
	n_entries(0),
	...
	__iterators(nullptr)
{
	...
	if(container.header.data_header.controller.mixedStride){
		...
		case(YON_TYPE_64B): func = &self_type::__getNextStride<U64>;  this->n_entries = container.buffer_strides_uncompressed.size() / sizeof(U64);  break;
		...
		if(container.header.data_header.controller.signedness){
			...
			case(YON_TYPE_64B): (this->__setup<S64>(container, func));  break;
			default: std::cerr << "Disallowed" << std::endl; return;
			}
		} else {
			switch(container.getDataPrimitiveType()){
			...
			case(YON_TYPE_64B): (this->__setup<U64>(container, func));  break;
			default: std::cerr << "Disallowed" << std::endl; return;
			}
		}
	} else {
