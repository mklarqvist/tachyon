...
public:
	...
	U64 iterate_all_info(std::ostream& stream = std::cout){
		...
		containers::InfoContainerInterface** its = new containers::InfoContainerInterface*[this->block.header.n_info_streams];
		...
		for(U32 p = 0; p < meta.size(); ++p){
			...
			if(this->block.header.n_filter_streams){
				const U32& n_filter_keys = this->block.header.filter_bit_vectors[meta.at(p).filter_pattern_id].n_keys;
			...
			if(this->block.header.n_info_streams){
				const U32& n_keys = this->block.header.info_bit_vectors[meta.at(p).info_pattern_id].n_keys;
