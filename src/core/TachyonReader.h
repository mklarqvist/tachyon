...
public:
	...
	bool iterateGT(std::ostream& stream = std::cout){
		...
		cost[0] = 1; cost[1] = 2; cost[2] = 4; cost[3] = 8;
		U64 total_cost = 0;
		std::cerr << this->block.gt_rle_container.getSizeCompressed() << '\t' << this->block.gt_rle_container.getSizeUncompressed() << std::endl;
		std::cerr << this->block.gt_simple_container.buffer_data_uncompressed.size() << std::endl;
